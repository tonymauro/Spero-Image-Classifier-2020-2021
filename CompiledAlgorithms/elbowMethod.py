...
class Elbow:
    ...
    def elbowMethod(self, X):
        ...
        def doKmean(q, k, X):
            #Run the KMean algorithm with the given K
        ...
        inertias = []
        # Iterates K clustering for different K values from 1 to 10 with threading applied
        threads = []
        K = range(1, 10)
        print("Testing optimal K-clusters...")
        # Creates thread for each k value and run the doKmean function with parameter k.
        tempK = []
        for k in K:
            tempK.append(k)
        ...
        # Each thread returns tuplet (distortion, inertia)
        while not q.empty():
            thing = q.get()
            distortions.append(thing[0])
            inertias.append(thing[1])
        # Seems to have a problem where the k = 2 thread would return before k = 1 thread
        # Checks inertia values and switches k = 1 and k = 2 if k = 2 inertia is less than k = 1 inertia
