...
class BIC:
    ...
    def customBIC(self, data, origData):
        ...
        for n_clusters in self.cluster_range:
            gmm = GaussianMixture(n_components=n_clusters, n_init=3, covariance_type='full')
            gmm.fit(data)
            clusters = []
            ...
            for i in range(n_clusters):
                ...
                log_sigma[i] = np.log(max(det, 0.01))
            bic = 0
            ...
            bic -= 4*n_clusters*n_clusters*n_clusters*np.sqrt(n_clusters) #additional penalty term to prevent overenumeration
            print(bic)
            bics.append(bic)
